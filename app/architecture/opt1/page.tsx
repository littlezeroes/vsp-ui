...
/* Option ① — Giữ nguyên nav, thêm Sinh lời card vào Home */
  ...
  return (
    <div className="min-h-screen bg-background flex justify-center">
      <div className="relative w-full max-w-[390px] min-h-screen bg-background text-foreground flex flex-col">
        ...
        {/* Bottom Nav — GIỮA NGUYÊN */}
        <Nav active={0} tabs={["Trang chủ", "Chuyển tiền", "QR", "Giao dịch", "Tài khoản"]} />
        <Label>① Giữ nguyên nav — thêm Sinh lời card vào Home</Label>
      </div>
    </div>
  )
}

function Nav({ active, tabs }: { active: number; tabs: string[] }) {
  return (
      ...
      <div className="flex items-center justify-around pt-[6px] pb-[26px]">
        {tabs.map((t, i) => t === "QR" ? (
          ...
          <div key={i} className="flex flex-col items-center gap-[3px]">
            <div className={`w-[22px] h-[22px] rounded-[6px] ${i === active ? "bg-foreground" : "bg-foreground/15"}`} />
            <span className={`text-[10px] font-medium ${i === active ? "text-foreground" : "text-foreground-secondary"}`}>{t}</span>
          </div>
        ))}
      </div>
