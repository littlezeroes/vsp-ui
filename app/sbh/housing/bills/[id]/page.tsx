...
export default function HousingBillsPage({
  ...
  const bills = getBillsByApartment(id)
    ...
    <div className="relative w-full max-w-[390px] min-h-screen bg-background text-foreground flex flex-col">
      ...
      <Header
        ...
        leading={
          <button
            type="button"
            onClick={() => router.push("/sbh/housing")}
              ...
              {bills.map((bill, idx) => {
                ...
                return (
                  <ItemListItem
                    ...
                    suffix={
                      <span className={`text-sm font-semibold ${badge.className}`}>
                        {bill.status === "overdue" ? "Qua han" : ""}
                      </span>
                    }
                    showChevron
                    divider={idx < bills.length - 1}
                    onPress={() =>
                      router.push(`/sbh/bill/${bill.id}?type=housing`)
                    }
                  />
                )
              })}
